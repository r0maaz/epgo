...
class InvoiceController extends Controller
{
 ...
 public function createex($customerId)
    {


            $customFields   = CustomField::where('module', '=', 'invoice')->get();
            $invoice_number = 000;//\Auth::user()->invoiceNumberFormat($this->invoiceNumber());
            $customers      =Customer::get();// Customer::where('created_by', \Auth::user()->creatorId())->get()->pluck('name', 'id');
            $customers->prepend('Select Customer', '');
            $category =ProductServiceCategory::where('type','=', 1)->get()->pluck('name', 'id'); //ProductServiceCategory::where('created_by', \Auth::user()->creatorId())->where('type', 1)->get()->pluck('name', 'id');
            $category->prepend('Select Category', '');
            $product_services = ProductService::get()->pluck('name', 'id');//ProductService::where('created_by', \Auth::user()->creatorId())->get()->pluck('name', 'id');
            $product_services->prepend('--', '');

            return view('invoice.createex', compact('customers', 'invoice_number', 'product_services', 'category', 'customFields', 'customerId'));
